...
object GradientDescent extends Logging {
  ...
  def runMiniBatchSGD(
    ...
    var previousWeights: Option[Tensor] = None
    var currentWeights: Option[Tensor] = None
    ...
    var regVal = updater.compute(
      weights, new Tensor(Array(weights.size)), 0, 1, regParam)._2
    ...
    while (!converged && i <= numIterations) {
      ...
      val (gradientSum, lossSum, miniBatchSize) = data.sample(false, miniBatchFraction, 42 + i)
        .treeAggregate((new Tensor(Array(n)), 0.0, 0L))(
          seqOp = (c, v) => {
...
//    val currentBDV = currentWeights.toBreeze.toDenseVector
    ...
    solutionVecDiff < convergenceTol * Math.max(currentWeights.norm, 1.0)
  }
